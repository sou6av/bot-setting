...
const {Message, StringSession, Image, Video} = require('./𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊/');
...
// Sql
const 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊DB = config.DATABASE.define('𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊Duplicated', {
    info: {
...
async function 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 () {
    ...
    var StrSes_Db = await 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊DB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('open', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊DB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('🐺 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 💌*\n    *- Eva*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n*𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊  Working as Private!😎*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using 𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊] --*' + 
                                    '\n*𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊 bir hata gerçekleşti!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⛔ HATA ÇÖZÜMLEME [𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊] ⛔*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⛔ ERROR ANALYSIS [𝐑𝐀𝐆𝐀𝐍𝐎𝐑𝐊] ⛔*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
