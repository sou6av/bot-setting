...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
        ...
        let banned = jid.find( Jid => Jid === message.jid);
        if(banned !== undefined) return
        if (!!message.mene.jid, fs.readFileSync('./uploads/Mention.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted : message.data, ptt: true})
        }
...
const array = ['Akshan','Anthass','Ayin','Monu','padikk','Logan','Luca','number','sticker','support','work','photo','Wait','Poli','Sed','Sourav','vibe','Ayye','Ee','gn','Enth','Bgm','Hello','myre','h
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
...
filtreler.map(
    async (filter) => {
        pattern = new RegExp(filter.dataValues.regex ?
            await message.client.sendMessage(message.jid,filter.dataValues.text, MessageType.text, {quoted: message.data});
        }
    }
);
